...
> [!info] Source file (v2)
> `visualization.py` &middot; 415 lines &middot; Activation-volume builder plus every renderer: light multi-angle 3D surface montage, 2x2 glass brain, stat-map slices, the interactive Plotly gray-to-red folded-pial 3D, and the view_img slice viewer.

## Connected notes
[[brain_regions]] &middot; [[app]]
...
def plot_brain_glass(nifti_img, threshold=0.08):
    ...
    """
    panels = [
        ("l", "Left Sagittal",      "anterior â—„ | â–º posterior"),
        ("r", "Right Sagittal",     "anterior â—„ | â–º posterior"),
        ("z", "Axial â€” Top-down",   "left â—„ | â–º right"),
        ("y", "Coronal â€” Front",    "left â—„ | â–º right"),
    ]
    rendered = [
    ...
    fig.suptitle(
        "Brain Region Affinity Map â€” Glass Brain  (X-ray Projections)",
...
def plot_brain_stat(nifti_img, threshold=0.08):
    """
    Two-row layout:
      Row 1 â€” 6 axial slices (horizontal cuts, top to bottom through the brain)
      Row 2 â€” 3 orthogonal cross-sections (sagittal / coronal / axial)
    ...
    disp_axial = plotting.plot_stat_map(
        ...
        cut_coords=8,
        colorbar=True,
        title="Axial Slices  (horizontal cuts, inferior â†’ superior)",
        **stat_kw,
    )
    ...
    ax1.axis("off")
    ax1.set_title(
        "Axial Slices  â€”  horizontal cuts through the brain (top to bottom)",
    ...
    fig.suptitle(
        "Brain Region Affinity Map â€” Anatomical Slices",
...
def interactive_surface_html(nifti_img, surf_mesh="fsaverage6", threshold=0.08,
    ...
    view = plotting.view_img_on_surf(
        ...
        vmax=1.0,
        colorbar=True,
        title="Region Affinity â€” interactive 3D surface",
    )
...
def interactive_volume_html(nifti_img, threshold=0.08, height=540, width=1000):
    ...
    movable cross-hair â€” good for deep / subcortical structures. Returns an HTML
    ...
    view = plotting.view_img(
        ...
        black_bg=False,
        colorbar=True,
        title="Region Affinity â€” interactive slices",
    )
    return view.get_iframe(width=width, height=height)
...
def interactive_surface_plotly(nifti_img, threshold=0.08, surf_mesh="fsaverage6",
    ...
    the *pial* surface, so the real gyri and sulci (ridges/folds) are visible â€”
    not the smoothed inflated balloon. The cortex is shaded gray by curvature
    (gyri light, sulci dark), and affected regions ramp gray â†’ yellow â†’ orange â†’
