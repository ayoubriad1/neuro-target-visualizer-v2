...
- CSV/TSV bulk import: one row per region, `region,kcal` columns (+ optional
  ...
  real virtual-screening output to visualize.
...
def parse_csv(text: str) -> CSVImportResult:
    ...
    for line_num, row in enumerate(reader, start=2):  # header occupies line 1
        ...
        if coordinates is not None:
            ...
            errors.append(f"Line {line_num}: missing region name (and no x/y/z given).")
            continue
        ...
        rows.append(ImportedRow(name=name, kcal=kcal, coordinates=coordinates))
