...
if st.session_state.regions:
    ...
    if view_mode == "Interactive 3D":
        loader.markdown(
            brain_loader_html("Building interactive 3D brain…"),
            unsafe_allow_html=True,
        )
        ...
        loader.markdown(
            brain_loader_html("Building interactive slice viewer…"),
            unsafe_allow_html=True,
        )
        ...
        loader.markdown(
            brain_loader_html("Rendering glass brain…"),
            unsafe_allow_html=True,
        )
        nifti_img = create_activation_volume(pairs, sigma=DEFAULT_SIGMA)
        ...
        loader.markdown(
            brain_loader_html("Rendering anatomical slices…"),
            unsafe_allow_html=True,
        )
        nifti_img = create_activation_volume(pairs, sigma=DEFAULT_SIGMA)
...
predicted binding affinity at **{top_kcal:.1f} kcal/mol** (≈ {top_norm:.0f} %
...
"""
    ...
    with st.expander("🔬 Methods & provenance (for reproducibility)", expanded=False):
        ...
        try:
            ...
            _vers = f"nilearn {_nl.__version__} · matplotlib {_mpl.__version__} · numpy {_np.__version__}"
        except Exception:
            _vers = "n/a"
...
| Region model | {len(get_region_names())} regions as single MNI points → Gaussian blobs |
| Activation σ | surface = {SURFACE_SIGMA} mm · glass/stat = {DEFAULT_SIGMA} mm |
| Cortical surface | `{_surf_mesh}` · {'inflated' if surf_inflate else 'pial'} |
| Colormap | YlOrRd (0 → 1 normalized intensity) |
| Display threshold | {threshold:.2f} |
| kcal/mol → % | linear over [{KCAL_MIN:.0f}, {KCAL_MAX:.0f}] |
| Renderer | {_vers} · Python {sys.version.split()[0]} |
| Generated | {datetime.datetime.now():%Y-%m-%d %H:%M} |

**Data provenance** — affinity values are **user-entered** (`kcal/mol`); v2 uses
